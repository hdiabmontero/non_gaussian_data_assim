...
#     # Calculate the posterior
    ...
    if N_eff < nc_threshold :
        ...
        epsc=np.random.normal(0,0.5,mem)
        for i in range(mem):
...
folder_truth='/data/bk_rsf_1d/BK_RSF_1D_test_datasets_chaotic/truth/'
...
# mu_pert = 0
# sigma_pert = np.sqrt(0.1)

# # Observations
# r_eps_theta=np.sqrt(0.1)
# r_eps_u=np.sqrt(0.1)

# r_eps_tau = np.sqrt(0.6)
# r_eps_vel = np.sqrt(0.6)

# mu_pert = 0
# pert_obs_theta = np.random.normal(mu_pert, r_eps_theta, [ len(t_obs), int(N/obs_den)])
# pert_obs_u = np.random.normal(mu_pert, r_eps_u, [ len(t_obs), int(N/obs_den)])
# pert_obs_v = np.random.normal(mu_pert, r_eps_vel, [ len(t_obs), int(N/obs_den)])
# pert_obs_tau = np.random.normal(mu_pert, r_eps_tau, [ len(t_obs), int(N/obs_den)])


# x_obset = np.ones((len(t_obs), 2)) * -999
# x_obset[:, 0] = t_obs
# x_obset[:,1] = index_truth[::obs_rate]

# theta_obset = np.ones((len(t_obs), N)) * -999
# u_obset = np.ones((len(t_obs), N)) * -999
# v_obset = np.ones((len(t_obs), N)) * -999
# tau_obset = np.ones((len(t_obs), N)) * -999

#theta_obset[:, 0::obs_den] = theta_truth[::obs_rate,0::obs_den] + pert_obs_theta
# u_obset[:, 0::obs_den] = u_truth[::obs_rate,0::obs_den] + pert_obs_u
# Perturbations from velocity to avoid negative values
# v_obset[:, 0::obs_den] = np.log(v_truth[::obs_rate,0::obs_den]+1) + pert_obs_v
# tau_obset[:, 0::obs_den] = tau_truth[::obs_rate,0::obs_den] + pert_obs_tau

t_first_da = int(warm_nt/obs_rate)+1 # index first assimilation 

folder_obsnet='/data/bk_rsf_1d/BK_RSF_1D_test_datasets_chaotic/obsnet/'
...
folder_prior='/data/bk_rsf_1d/BK_RSF_1D_test_datasets_chaotic/prior/'
...
#     K_gain_t[:, :, d]=post_agmf['kalman_gain']
   ...
   # Update theta
    ...
    #list_kurtosis.append(kurtosis(X_da_posterior,axis=1))
    #list_skewness.append(skew(X_da_posterior,axis=1))
...
filename_estimate_theta='./results_rmse_test/rmse_theta_bk1d_chaotic_obs_den_'+str(obs_den).zfill(2)+'_run_'+name_exp+'.txt'
...
filename_estimate_u='./results_rmse_test/rmse_u_bk1d_chaotic_obs_den_'+str(obs_den).zfill(2)+'_run_'+name_exp+'.txt'
...
filename_estimate_v='./results_rmse_test/rmse_v_bk1d_chaotic_obs_den_'+str(obs_den).zfill(2)+'_run_'+name_exp+'.txt'
...
filename_estimate_tau='./results_rmse_test/rmse_tau_bk1d_chaotic_obs_den_'+str(obs_den).zfill(2)+'_run_'+name_exp+'.txt'
