...
#     # Perturbed observation vector
    ...
    for i in range(mem):
        for j in range(num_obs):
            eps_vect[j,i]=np.random.normal(0,r_obs_vect[j],1)
            obs_vect_perturbed[j,i]=obs_vect_filtered[j,0]+eps_vect[j,i]  
...
# eps_vector=np.random.normal(0,0,n_mem)
...
while t*tstep < t_assim*tstep:
    ...
    #y_t[0:N, 0][index_obs] = theta_obsnet[d,:][index_obs] # Observations theta
    ...
    post_enkf = enkf(n_mem, (n_x+1)*N, X_da_prior, y_t, R)
